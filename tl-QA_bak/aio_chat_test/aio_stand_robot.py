...
async def chat_room(live_info: dict, headers: dict, job_num: int):
    ...
    chat_user.is_stay = True
    chat_user.is_gift = True
    ...
    while True:
        await gen.sleep(random.randint(60, 3600))
        qq = random.randint(0, 100)
        if qq >= 90:
            if chat_user.is_gift:
                await chat_user.send_gift(
                    random.choice(options.gift_ids), live_info.get("MasterId")
                )
                is_gift = False
        else:
            msg = random.choice(options.msg_list)
            logging.info(msg)
            await chat_user.send_message(msg, chat_user.room_id)
        leave_q = random.randint(0, 100)
        if leave_q >= 90:
