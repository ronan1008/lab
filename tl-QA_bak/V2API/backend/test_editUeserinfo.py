...
class TesteditUserinfo():
    ...
    def setup_class(self):
        ...
        self.idlist.append(api.search_user(test_parameter['prefix'], 'track0010', header))
    ...
    def testShowUserinfo_case1(self):
        #回傳的資訊，新增大頭照
        apiName = '/api/v2/backend/user/'
        header['X-Auth-Nonce'] = test_parameter['backend_nonce']
        header['X-Auth-Token'] = test_parameter['backend_token']
        ...
        #有大頭照的 User
        apiName_1 = apiName + self.idlist[4]
        res = api.apiFunction(test_parameter['prefix'], header, apiName_1, 'get', None)
        restext = json.loads(res.text)
        pprint(restext)
        assert res.status_code // 100 == 2
        assert restext['profilePicture'] == "https://d1a89d7jvcvm3o.cloudfront.net/personal/d3ca9f415fe26acbf08af77db19e6f70.jpeg"

        #沒有大頭照的 User
        apiName_2 = apiName + self.idlist[2]
        res = api.apiFunction(test_parameter['prefix'], header, apiName_2, 'get', None)
        restext = json.loads(res.text)
        pprint(restext)
        assert res.status_code // 100 == 2
        assert restext['profilePicture'] == 'https://d1a89d7jvcvm3o.cloudfront.net/personal/fadbe97ffce8c3b940e647ced2c81c87.jpeg'
        #assert restext['identityStatus'] == 1
