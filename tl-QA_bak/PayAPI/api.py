...
def create_photo_comment(prefix, header1, pid, comment):
    url = prefix + '/api/v1/photo/comment/' + pid + '/create'
...
def upload_image(prefix, header1, photo_path, file_name, file_addr):
    url = prefix + '/api/v2/cdn/' + photo_path + '/photo'
    del header1['Content-Type']
...
def operator_photopost(prefix, header1, way, post_id, content):
    url = prefix + '/api/v2/liveMaster/photoPost/' + post_id    
    if not content:
        if way == 'get':
            res = requests.get(url, headers=header1)
        else:
