...
def request_url_to_dict(url):
	...
	r = requests.get(_url_json_link(url))
...
def main():
	...
	films_dict = { film_info['episode_id'] : film_info['title'] for film_info in films_list }
	...
	vehis_speed_df = pd.DataFrame(vehis, columns=['speed'], index =vehis['name'])
	print(vehis_speed_df)
	ax = vehis_speed_df.plot(title='Vehicle of Star Wars speed over 1000', kind='barh', fontsize=10, figsize=(15,5))
	for i in ax.patches:
		ax.text(i.get_width()+100, i.get_y()+0.15, i.get_width(), fontsize=10, color='red') 
	plt.ylabel('Vehicles')
	plt.xlabel("Speed")
	plt.show()
